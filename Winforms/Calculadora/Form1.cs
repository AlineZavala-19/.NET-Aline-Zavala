...
public partial class Form1 : Form
{
    ...
    private void btnDec_Click(object? sender, EventArgs e){
        ...
        int numero = 0;
        int digito = 0;
        long binario = long.Parse(txtDisplay.Text);
        for(long inicio = binario, j = 0; inicio > 0; inicio/=10, j++){
            digito = (int)inicio%10;
    ...
    private void btnSin_Click(object? sender, EventArgs e){
        lblOperacion.Text = $"Seno({txtDisplay.Text}) =";
        txtDisplay.Text = Math.Sin(Double.Parse(txtDisplay.Text)).ToString();
    }
    private void btnHex_Click(object? sender, EventArgs e){
        int numero = 0;
        const int DIVISOR = 16;
        String hexadecimal = Convert.ToString(Int32.Parse(txtDisplay.Text)).ToString();
        for (int inicio = 0, j = hexadecimal.Length - 1; inicio < hexadecimal.Length; inicio++, j--)
        {
    
            if (hexadecimal[inicio] >= '0' && hexadecimal[inicio] <= '9')
            {
                numero += (int)Math.Pow(DIVISOR, j) * Convert.ToInt32(hexadecimal[inicio] + "");
            } else if (hexadecimal[inicio] >= 'A' && hexadecimal[inicio] <= 'F') {
                numero += (int)Math.Pow(DIVISOR, j) * Convert.ToInt32((hexadecimal[inicio] - 'A' + 10) + "");
            }
            else
            {
                txtDisplay.Text = "-1";
